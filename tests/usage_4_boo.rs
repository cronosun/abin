...
use abin::{Bin, BinBuilder, BinFactory, BooStr, NewBin, NewStr, Str, StrBuilder, StrFactory, StrSegment, ToBooConverter};
...
fn entity_with_borrowed_or_owned_fields() {
    ...
    assert_eq!(first_name, extract_first_name_owned(entity_borrowed_first_name, "Muster"));
    assert_eq!(first_name, extract_first_name_owned(entity_owned_first_name, "Muster"));
}

fn extract_first_name_owned<'a>(entity: StrEntity, expected_last_name: impl Into<BooStr<'a>>) -> Str {
...
fn boo_in_hash_map() {
    ...
    map.insert(NewStr::from_static("entry1").owned(), NewStr::from_static("Value for entry1").owned());
    map.insert(NewStr::from_static("entry2").owned(), NewStr::from_static("Value for entry2").owned());
    map.insert(NewStr::from_static("entry3").owned(), NewStr::from_static("Value for entry3").owned());
